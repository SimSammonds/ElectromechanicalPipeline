% EndoFLIP data analysis - Electrocontractile Coupling for APC002.
...
% Pyloric squeeze: 11:34:10 (T2) (no cradle)
% Start time: 600s (BDF) / 4439 (T3)
% End time: 680s (BDF) / 5239 (T3)

Fs_flip = 10; % Sampling frequency (Hz)
% Extract and save to matrix:
exp2 = data_m3(4439:5239,4:20); % 1-16: Dest, 17: Pressure
[nRows,~] = size(exp2);

squeeze = data_m2(32000:end,4:20);
[R_sqz,C_sqz] = size(squeeze);
...
figure(); % Visualise Squeeze
t_sqz = (0:R_sqz-1)*(1/Fs_flip);
plot(t_sqz,squeeze(:,1:16));
...
if import2 == 1
    ...
    toappSWdata = load('APC002_exp2_600-680s_SW_elec3','toapp');
    toappSPKdata = load('APC002_exp2_600-680s_SPK_elec3','toapp');
    excelCONdata = readmatrix('APC002_ContractionClusterList.xlsx');
    SW_AMPs = readmatrix('APC002_SW_AMPs.xlsx');
...
g_clsts = [60:64];
i_clsts = [31:33;36:38;40:42;45:47;49:51];
...
i_clsts = 29:53; % Add all intestinal slow wave clusters to i_list
