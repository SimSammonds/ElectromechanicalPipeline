% EndoFLIP data analysis - Electrocontractile Coupling for APB040.
...
%% Begin with probing - experiment 4

% Start time: 11:02 am - End time: 11:06 am
% Entry - Data 3: 4775 (start of 11:02) - 7764 (end of 11:06)

Fs_flip = 10; % Sampling frequency (Hz)
% Extract and save to matrix:
probe = data_m3(4775:7764,4:20); % 1-16: Dest, 17: Pressure
...
% pressure peaks
shifter = 27; % The amount to shift the points in time (s)
...
%% Baseline temporal correlation - experiment 3

% Start time: 10:39 am - End time: 10:57 am
% Entry - Data 2: 27033 (start of 10:39) - end (to be cont.)
% Entry - Data 3: 1 (cont.) - 2384 (end of 10:57)

Fs_flip = 10; % Sampling frequency (Hz)
% Extract and save to matrix:
probe1 = data_m2(27033:end,4:20); % 1-16: Dest, 17: Pressure
probe2 = data_m3(1:2384,4:20);
probe = [probe1; probe2];
[nRowsP,nColsP] = size(probe);
...
% Superimpose BDF time stamps onto plot and shift until they line up with
% pressure peaks
shifter = -90; % The amount to shift the points in time (s)
start_time = 0; % Below times are subtracted to find relative time
points_s = [426,1110]; % (s) start points
points_e = [457,1141]; % (s) end points
points_s = points_s - start_time + shifter;
points_e = points_e - start_time + shifter;
...
% In conclusion:
% BDF time needs to be shifted by 90 seconds ->
% EndoFLIP time occurs 90 seconds BEFORE BDF time.
...
expDATA1 = data_m2(27033:end,4:20); % 1-16: Dest, 17: Pressure
expDATA2 = data_m3(1:2384,4:20);
flipDATA = [expDATA1; expDATA2];
[nRowsExp,nColsExp] = size(flipDATA);

% Find pylorus (most common minimum CSA):
for row = 1:nRowsExp % Pylorus == minimum CSA on EndoFLIP
    minimum = min(flipDATA(row,1:16)); % Save as index 18
    flipDATA(row,19) = find(flipDATA(row,1:16)==minimum,1);
    flipDATA(row,18) = minimum;
end
pyl_flip = mode(flipDATA(:,19)); % Find most common sensor with minCSA
pyl_flip = 10; % Manual override - 
% 10 is the 2nd most common MinCSA and is more expected than sensor 15

% Shift entries such that 0 seconds (BDF) = 1st entry (exp4 data)
% If shifting FLIP file back (will need to extract raw again for forward):
% expDATA_shft = flipDATA(1+(shifter*Fs_flip):end,:);
expDATA_shft1 = data_m2((27033+(shifter*Fs_flip)):end,4:20);
expDATA_shft2 = data_m3(1:(2384+(shifter*Fs_flip)),4:20); % 1-16: Dest, 17: Pressure
expDATA_shft = [expDATA_shft1; expDATA_shft2];
[nRows, nCols] = size(expDATA_shft);

% USER SETTINGS:
wind = [0,200]; % seconds - MAKE SAME AS SELECTED BDF TIMES
viewElecs = 1:16;
...
for plt = 1:length(viewElecs)
    ...
    plot(t_exp, expDATA_shft(:,viewElecs(plt)))
    xlim(wind);
...
import2 = 1;
if import2 == 1
    toappSWdata = load('APB040_exp3_10-110s_SW_elec3','toapp');
    toappSPKdata = load('APB040_exp3_10-60s_SPK_elec3','toapp');
    excelCONdata = readmatrix('APB040_ContractionClusterList.xlsx');
    SW_AMPs = readmatrix('APB040_SW_AMPs.xlsx');
...
g_clsts = [1:5];
i_clsts = [11:13;15:17;18:20;21:23;24:26];
...
i_clsts = 10:26; % Changing the list to incorporate all intestinal waves

% Extract valid contractions & metrics:
[CONdata, ~] = ValidateContractions(flipConfig,excelCONdata,expDATA_shft,t_exp);
...
CON_SW_metrics = [];
for CONclst = CONs'
    metrics_temp = ContractionMetrics(CONclst,flipConfig,CONdata,expDATA_shft,t_exp);
...
% 
% p_elecs2 = [132 129 125 121 117 113 105 101 130 126 122 118 114 110 106 102 131 127 123 119 115 111 107 103 128 124 120 116 112 108 104];
% 
% % Initialise tallies
% sd_tally = 0;
% d_tally = 0;
