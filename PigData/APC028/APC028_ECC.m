% EndoFLIP data analysis - Electrocontractile Coupling for APC027.
...
% BDF zero: 0s (BDF) / 15906 (flip entry)
% Start time: 800s (BDF) / 23906 (flip entry)
% End time: 1000s (BDF) / 25906 (flip entry)

% Pairing: 11.10.30 -> 401 BDF -> entry # 19916

% Extract and save to matrix:
expB = CON_data_m(23901:25901,4:20); % 1-16: Dest, 17: Pressure
[nRows,~] = size(expB);

% USER SETTINGS:
wind = [120,200]; % seconds
viewElecs = 2:16;
...
for row = 1:nRows % Pylorus == minimum CSA on EndoFLIP
    minimum = min(expB(row,9:12)); % Save as index 18
    expB(row,19) = find(expB(row,9:12)==minimum,1);
    expB(row,18) = minimum;
end
pyl_flip = mode(expB(:,19)); % Find most common sensor with minCSA
pyl_flip = 9; % Manual override
...
if import2 == 1
    addpath 'F:\PigData\APC028'
    toappSWdata = load('APC028_exp1_800-1000s_SW','toapp');
    toappSPKdata = load('APC028_exp1_800-1000s_SPK','toapp');
    excelCONdata = readmatrix('APC028_ContractionClusterList.xlsx');
    SW_AMPs = readmatrix('APC028_SW_AMPs.xlsx');
...
g_clsts = 1:13;
i_clsts = [15:17;19:21;22:24;28:30;31:33;37:39;42:44;45:47;49:51;56:58;60:62;64:66;69:71];
