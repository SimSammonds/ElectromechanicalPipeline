...
% BDF zero: 0s (BDF) / 16313 (flip entry)
% Start time: 800s (BDF) / 20813 (flip entry)
% End time: 1000s (BDF) / 22813 (flip entry)

% Pairing: 11.16.00 -> 1367 BDF -> entry # 29983

% Extract and save to matrix:
expB = CON_data_m(20813:22813,4:20); % 1-16: Dest, 17: Pressure
[nRows,~] = size(expB);

% USER SETTINGS:
wind = [155,200]; % seconds
viewElecs = 8:16;
...
for row = 1:nRows % Pylorus == minimum CSA on EndoFLIP
    minimum = min(expB(row,2:16)); % Save as index 18
    expB(row,19) = find(expB(row,2:16)==minimum,1);
    expB(row,18) = minimum;
end
...
if import2 == 1
    addpath 'F:\PigData\APC029'
    toappSWdata = load('APC029_exp1_450-650s_SW','toapp');
    toappSPKdata = load('APC029_exp1_450-650s_SPK','toapp');
    excelCONdata = readmatrix('APC029_ContractionClusterList.xlsx');
    SW_AMPs = readmatrix('APC029_SW_AMP.xlsx');
...
g_clsts = 1:13;
i_clsts = [16:18;20:22;23:25;27:29;34:36;39:41;43:45;48:50;51:53;55:57;58:60;65:67;69:71];
