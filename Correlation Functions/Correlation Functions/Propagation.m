function [OUTPUT] = Propagation(SWdata,SWtoapp,SPKdata,elecConfig,oldElecConfig,g_clts,i_clts,QZWidth)
...
% Define organ per electrode,
% SWtoapp contains truth data:
[g_list,i_list,p_list] = TrueChans(SWtoapp,g_clts,i_clts);
[p_list] = ConfirmPlist(p_list,elecConfig,oldElecConfig,QZWidth);

% For SPKs, do we want to include SPKs which traverse organ boundaries? Do
% these count as BOTH, ONE, or NEITHER? Provide different options and see
...
% For each SW
SWclts = unique(SWdata(:,1));
for curClst = SWclts'
    
    % Find all elecs influenced
    curIdx = find(SWdata(:,1) == curClst);
    curElecs = SWdata(curIdx,2);
    curATs = SWdata(curIdx,3);
    
    % Create activation matrix for this SW
    [r,c] = size(oldElecConfig);
    ATMatrix = NaN(r,c);
    ...
    for i = 1:r
        for j = 1:c
            ...
            if ismember(elecLabel,curElecs)
                idx = find(curElecs == elecLabel);
                ATMatrix(i,j) = curATs(idx);
                
                % Saving max and min activations
                if elecLabel < 161 % Anterior only - wrapping too difficult to manage
    ...
    % Determine propagation distances for this wave
    Px = abs(furEast - furWest) * 5;
    Py = abs(highest - lowest) * 5;
    ...
    if ismember(curClst,g_clts)
        SW_g_V = [SW_g_V; Vx, Vy, Vm];
        SW_g_P = [SW_g_P; Px, Py];
    elseif ~isnan(Vx) && ~isnan(Vy) && ~isnan(Vm)
...
% For each SPK
SPKclts = unique(SPKdata(:,1));
for curClst = SPKclts'
    
    % Find all elecs influenced
    curIdx = find(SPKdata(:,1) == curClst);
    curElecs = SPKdata(curIdx,2);
    curATs = SPKdata(curIdx,3);
    ...
    % Determine propagation distances for this wave
    if furEast == 999; furEast = 0; Px=[]; Py=[];
    elseif furWest == 999; furWest = 0; Px=[]; Py=[];
    else
        Px = abs(furEast - furWest) * 5;
        Py = abs(highest - lowest) * 5;
    end
    ...
    % Find most common region/organ
    % Find number of times this spk appears in gastric electrodes
    g_times = numel(intersect(curElecs,g_list));
    i_times = numel(intersect(curElecs,i_list));
    p_times = numel(intersect(curElecs,p_list));
    
    % Categorise wave by region
    if ~isnan(Vx)
        if (g_times >= i_times) && (g_times >= p_times)
            SPK_g_V = [SPK_g_V; Vx, Vy, Vm];
            SPK_g_P = [SPK_g_P; Px, Py];
        elseif (i_times >= g_times) && (i_times >= p_times)
            SPK_i_V = [SPK_i_V; Vx, Vy, Vm];
            SPK_i_P = [SPK_i_P; Px, Py];
        elseif (p_times >= i_times) && (p_times >= g_times)
            SPK_p_V = [SPK_p_V; Vx, Vy, Vm];
            SPK_p_P = [SPK_p_P; Px, Py];
        end
    end
end

% Collate data for output
SW_velo = [mean(SW_g_V);mean(SW_i_V)];
% SW_velo = [mean(SW_g_V);[1,2,3]]; % Just for APC007 (no iSWs)
SW_prop = [mean(SW_g_P);mean(SW_i_P)];
% SW_prop = [mean(SW_g_P);[1,2]]; % Just for APC007 (no iSWs)

SPK_velo = [mean(SPK_g_V);mean(SPK_p_V);mean(SPK_i_V)];
SPK_prop = [mean(SPK_g_P);mean(SPK_p_P);mean(SPK_i_P)];
% SPK_velo = [mean(SPK_g_V);mean(SPK_p_V);[1,2,3]]; % Just for APC007 and APC009 (no iSWs)
% SPK_prop = [mean(SPK_g_P);mean(SPK_p_P);[1,2]]; % Just for APC007 and APC009 (no iSWs)
...
OUTPUT = [SW_prop(1,2), NaN, SW_prop(2,2);...
         SW_velo(1,2), NaN, SW_velo(2,2);...
         SW_prop(1,1), NaN, SW_prop(2,1);...
         SW_velo(1,1), NaN, SW_velo(2,1);...
         SPK_prop(1,2),SPK_prop(2,2),SPK_prop(3,2);...
         SPK_velo(1,2), SPK_velo(2,2), SPK_velo(3,2);...
         SPK_prop(1,1),SPK_prop(2,1),SPK_prop(3,1);...
         SPK_velo(1,1), SPK_velo(2,1), SPK_velo(3,1)];
